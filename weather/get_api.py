...
def find_closest_city(lat, long, acceptable_distance=15, model_type='current'):
    ...
    for city in queryset:
        distance = geo_distance(
            origin_coordinates, (city.latitutde, city.longitude))
...
def clean_names_current(json_dict):
    ...
    transformed_dict['longitude'] = json_dict['coord']['long']
    ...
    transformed_dict['temprature'] = json_dict['main']['temp']
    transformed_dict['temprature'] = json_dict['main']['temp']
    ...
    transformed_dict['could'] = json_dict['cloud']['all']
    ...
    transformed_dict['calculation_datetime'] = json_dict['dt']
    transformed_dict['city_country'] = json_dict['sys']['country']
    transformed_dict['city_id'] = json_dict['sys']['id']
...
def clean_names_predict(json_dict):
    ...
    cleaned_data['longitude'] = json_dict['city']['coord']['long']
    cleand_data['city_id'] = json_dict['city']['id']
    cleand_data['data'] = json_dict['list']
    return cleaned_data
...
def get_create_current_weather(lat, long, api_key=WEATHER_API_KEY, delay=1):
    ...
    if closest_city_id != None:
        ...
        if curr_time - closest_city.update_time < delta:
            # data is less than one hour old
            return closest_city
        else:
            # data is more than one hour old
            json_data = requests.get(current_id.format(city_id=closest_city_id, api_key=WEATHER_API_KEY))
            json_dict = json.loads(json_data.text)
            cleaned_data = clean_names_current(json_dict)
            # update date or make new data
            closest_city, bool_created = InternationalCurrent.update_or_create(city_id=cleaned_data['city_id'], defaults=cleaned_data)
            return closest_city
    else: # if closest city ID is None:
        json_data = requests.get(current_lat_long.format(lat=lat, long=long, api_key=WEATHER_API_KEY))
        json_dict = json.loads(json_data.text)
        cleaned_data = clean_names_current(json_dict)
...
def get_create_predict_weather(lat, long, api_key=WEATHER_API_KEY, delay=1):
    ...
    if closest_city_id != None:
        ...
        if curr_time - closest_city.update_time < delta:
            ...
            # data is more than one hour old
            json_data = requests.get(predict_id.format(city_id=closest_city_id, api_key=WEATHER_API_KEY))
            json_dict = json.loads(json_data.text)
            cleaned_data = clean_names_predict(json_dict)
            # update date or make new data
            closest_city, bool_created = GlobalPredict.update_or_create(city_id=cleaned_data['city_id'], defaults=cleaned_data)
            return closest_city
    else:  # if closest city ID is None:
        json_data = requests.get(predict_lat_long.format(lat=lat, long=long, api_key=WEATHER_API_KEY))
        json_dict = json.loads(json_data.text)
        cleaned_data = clean_names_predict(json_dict)
        # update or create city weather data
        closest_city, bool_created = GlobalPredict.update_or_create(city_id=cleaned_data['city_id'], defaults=cleaned_data)
